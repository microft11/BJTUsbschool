...
def get_lines(edge_img):
    ...
    if lines is None:  # 检查 lines 是否为 None
        return None, None
    ...
    for line in lines:
        for x1, y1, x2, y2 in line:
            k = (y2 - y1) / (x2 - x1)
            if k > -0.4:  # 斜率小于-0.2的是左边的线
                left_lines.append(line)
            elif k < 1.3:  # 斜率大于1.6的是右边的线
...
def draw_line(img, lines):
    if lines[0] is not None:
        cv2.line(img, tuple(lines[0][0]), tuple(lines[0][1]), color=(100, 205, 255), thickness=2)
    if lines[1] is not None:
        cv2.line(img, tuple(lines[1][0]), tuple(lines[1][1]), color=(100, 205, 255), thickness=2)
