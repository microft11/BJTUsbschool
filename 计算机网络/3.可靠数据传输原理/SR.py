...
class SelectiveRepeat:
    def __init__(self, window_size, total_packets):
        ...
        self.next_seq_num = 0
        self.base = 0
        self.acked = [False] * total_packets
    ...
    def receive_ack(self, packet):
        ...
        if random.random() < LOSS_PROBABILITY:
            ...
            self.lock.acquire()
            self.acked[packet] = True
            while self.base < self.total_packets and self.acked[self.base]:
                self.base += 1
            self.lock.release()
...
# 示例使用
sr = SelectiveRepeat(window_size=4, total_packets=10)
sr.send()
